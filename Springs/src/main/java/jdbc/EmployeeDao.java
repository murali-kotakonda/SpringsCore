...
public class EmployeeDao {
	...
	public int saveEmployee1(Employee e) {
		int count = jdbcTemplate.update("insert into employeeinfo values(?,?,?)",
	...
	public int updateName(Employee e) {
		...
		int count = jdbcTemplate.update(query, new Object[] { e.getName(), e.getId() });
		return count;
	}
	...
	/**
	 ...
	 sql :delete from employeeinfo where name=?
	 */
	...
	*/
	...
	// for ResultSetExtractor dev has to create the list ,
	//dev has to keep the object inside the list by looping the ResultSet
    ...
    steps for RowMapper:
	--------------
	//from rows to list   =>use RowMapper
	//no need to write loop
	//no need to create list
	//no need to add obj inside list
	...
	public Employee getEmpById(int id) {
		...
		List<Employee> query = jdbcTemplate.query("select * from employeeinfo where id =?", 
				new Object[] { id }, 
				mapper);
